...
def create_manual_control_widgets(parent, app):
    ...
    ).grid(row=0, column=1, pady=(0, 5), padx=(0, 5), sticky="e")

    # Nút Z↑ (Hàng 0, Cột 2)
    customtkinter.CTkButton(
        joystick_buttons_frame, text="Z \u2191", command=lambda: app.jog_axis("Z", -1), width=60, height=40, font=("Segoe UI", 16)
    ).grid(row=0, column=2, pady=(0, 5), padx=(5, 0), sticky="w")

    # Nút X← (Hàng 1, Cột 0)
    customtkinter.CTkButton(
        joystick_buttons_frame, text="\u2190 X", command=lambda: app.jog_axis("X", -1), width=60, height=40, font=("Segoe UI", 16)
    ).grid(row=1, column=0, padx=(0, 5), sticky="e")

    # Nút X→ (Hàng 1, Cột 4) -- Cột cuối cùng
    customtkinter.CTkButton(
        joystick_buttons_frame, text="X \u2192", command=lambda: app.jog_axis("X", 1), width=60, height=40, font=("Segoe UI", 16)
    ).grid(row=1, column=4, padx=(5, 0), sticky="w")

    # Nút Y↓ (Hàng 2, Cột 1)
    customtkinter.CTkButton(
        joystick_buttons_frame, text="Y \u2193", command=lambda: app.jog_axis("Y", -1), width=60, height=40, font=("Segoe UI", 16)
    ).grid(row=2, column=1, pady=(5, 0), padx=(0, 5), sticky="e")

    # Nút Z↓ (Hàng 2, Cột 2)
    customtkinter.CTkButton(
        joystick_buttons_frame, text="Z \u2193", command=lambda: app.jog_axis("Z", 1), width=60, height=40, font=("Segoe UI", 16)
    ).grid(row=2, column=2, pady=(5, 0), padx=(5, 0), sticky="w")
...
def create_program_widgets(parent, app):
    ...
    customtkinter.CTkLabel(manual_add_frame, text="X:").grid(
        row=0, column=0, padx=(10, 2), pady=10
    )
    customtkinter.CTkEntry(
        manual_add_frame, textvariable=app.manual_pos_entry["X"], width=70
    ).grid(row=0, column=1)
    customtkinter.CTkLabel(manual_add_frame, text="Y:").grid(
        row=0, column=2, padx=(10, 2), pady=10
    )
    customtkinter.CTkEntry(
        manual_add_frame, textvariable=app.manual_pos_entry["Y"], width=70
    ).grid(row=0, column=3)
    customtkinter.CTkLabel(manual_add_frame, text="Z:").grid(
        row=0, column=4, padx=(10, 2), pady=10
    )
    customtkinter.CTkEntry(
        manual_add_frame, textvariable=app.manual_pos_entry["Z"], width=70
    ).grid(row=0, column=5)
    customtkinter.CTkButton(
        manual_add_frame, text="Add Manual", command=app.add_manual_waypoint
    ).grid(row=0, column=6, padx=10, pady=10, sticky="ew")
